        ...
        <div class="wrapfixed">
          ...
          <div class="card">
            <div class="card-header">
              <span class="font-weight-bold text-info">Users list (Staff)</span>
            </div>
            <!--card-body-->
                  <div class="card-body border-bottom">

                    <div class="p-1 mb-2">
                      <a href="userlistStd.php" class="btn border mr-1">Student list</a>
                      <a href="userlistStaff.php" class="btn border mr-3 bg-info text-light">Staff list</a>
                      <input type="text" name="search" placeholder="Search a staff..." class="form-control" style="display: inline;width: auto;" id="liveSearch">
                    </div>
...
$query = "SELECT * FROM lm_user WHERE person='staff' ORDER BY id DESC LIMIT $start_from, $per_page";
...
if ($query != false) {
  ...
  while ($result = $query->fetch_assoc()) { $i++; ?>
                          <tr class="tr">
                            ...
                            <td><a class="mr-3" href="editUserStaff.php?editStaff=<?php echo $result['id'];?>&action=editUserStaff"><i class="fas fa-edit" style="font-size:25px;"></i></a>

                              <a onclick="return confirm('Are you sure to delete the user?');" href="delete.php?delUserStaff=<?php echo $result['id'];?>&deluname=<?php echo $result['uname'];?>&action=delStaff"><i class="fas fa-trash-alt" style="font-size:25px;"></i></a></td>
...
<?php }
  ...
  echo "<tr><td colspan='9'><p class='text-danger font-weight-bold text-center'>No user!</p></td></tr></tbody></table></div>";
}
?>
<!--End of pagination-->
